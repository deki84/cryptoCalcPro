...
export default function WatchlistPage() {
  ...
  // Favorites list (IDs -> coin objects)
  const favorites = useMemo(() => {
    const map = new Map(coins.map((c) => [c.id, c]));
    return ids.map((id) => map.get(id)).filter(Boolean) as MarketCoin[];
  }, [ids, coins]);

  // Keep selected coin valid
  useEffect(() => {
    if (!selectedCoinId) return;

    // if selected coin doesn't exist anymore, fallback
            ...
            <ul className="divide-y divide-zinc-800/60">
              {filtered.map((c) => (
                <li
                  key={c.id}
                  className={[
                    "py-3 flex items-center justify-between gap-4 cursor-pointer",
                    selectedCoinId === c.id ? "bg-white/5 -mx-2 px-2 rounded-lg" : "",
                  ].join(" ")}
                ...
                >
                  <div className="flex items-center gap-3 min-w-0">
                    ...
                    <div className="min-w-0">
                      ...
                      <div className="text-xs text-zinc-500">
                        Rank {c.rank ?? "—"} · {c.price.toLocaleString(undefined, { maximumFractionDigits: 2 })}{" "}
                        {fiat}
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <WatchListButton active={has(c.id)} onToggle={() => toggle(c.id)} size="md" />
                  </div>
                </li>
              ))}

              {!loading && filtered.length === 0 ? (
                <li className="py-6 text-sm text-zinc-500">No coins match your search.</li>
              ) : null}
            </ul>
          </div>
        </div>

        {/* Right: Selected chart + favorites preview */}
        <div className="flex flex-col gap-6">
